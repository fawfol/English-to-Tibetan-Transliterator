...
# 1. MODIFIED TRANSLITERATE FUNCTION
def transliterate(roman_string):
    ...
    while i < length:
        ...
        # Use a more robust transliteration method
        ...
        for key in SORTED_KEYS:
            if roman_string.startswith(key, i):
                ...
                # If it's a vowel, it modifies the previous character
                if key in "iuoe" and tibetan_output:
                    ...
                    else: # It's a standalone vowel, so add the 'ཨ' base
        ...
        if not found_match:
            # If no match found, just skip the character
            i += 1

    return tibetan_output


# 2. REWRITTEN SUGGESTION LOGIC
def get_suggestions(current_input):
    ...
    # Check if the typed word is a base consonant that can be modified
    if last_word in TIBETAN_MAP:
        base_tibetan = TIBETAN_MAP[last_word]
        suggestions = []

        # Generate Prefixed (ngon jug) suggestions
...
def update_on_input(roman_string):
    ...
    for i in range(MAX_SUGGESTIONS):
        if i < len(suggestions):
            # The full suggestion string now holds all necessary info
            # e.g., "ka:Prefix ག: གཀ"
            full_suggestion_data = suggestions[i]
            display_text = full_suggestion_data.split(':')[1] # "Prefix ག"
            tibetan_value = full_suggestion_data.split(':')[2].strip() # "གཀ"
            
            # The button's visible text is the explanation, its value is the data
...
# 3. MODIFIED BUTTON CLICK HANDLER
def update_input_with_suggestion(current_input, suggestion_data):
    ...
    roman_trigger = suggestion_data.split(':')[0]       # "ka"
    tibetan_value = suggestion_data.split(':')[2].strip() # "གཀ"
    ...
    new_input = tibetan_value.join(parts) + " " # Add space to complete the syllable
...
with gr.Blocks(theme=gr.themes.Soft()) as demo:
    gr.Markdown(
        """
        # Tibetan Transliteration Tool Tibetan Transliteration Tool 
        Type a Roman syllable to see its transliteration. For base consonants suggestions for prefixed and stacked variations will appear
        """
    )
    ...
    with gr.Row():
        roman_input = gr.Textbox(
            label="Roman Input",
            placeholder="Type Wylie here... e.g., 'tashi delek' or 'ka'",
            lines=4
        )
